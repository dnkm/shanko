...
const PHASES = [
  ...
  { phase: "initialize bank", anims: ["initial bank"] },
  { phase: "betting", anims: ["bet"] },
  { phase: "deal", anims: ["deal"] },
  { phase: "auto-shan", anims: ["auto-shan"] },
...
class Room {
  ...
  ready(user, socket, io) {
    ...
    this.piggyback(io);
    if (this.readyCheck()) this.nextPhase(io);
  }
  ...
  bet(data, user, socket, io) {
    if (this.phaseIndex !== 2 || !this.checkPlayer(user.sid)) return;
    for (let i = 0; i < this.players.length; i++)
  ...
  playerAction(data, user, socket, io) {
    if (this.phaseIndex !== 5 || this.checkPlayer(user.sid)) return;
